...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Partner
      .findOne(_.pick(params, _.keys(Partner.schema.paths)))
      .populate(populate);
  },

  /**
   * Promise to count partners.
   *
   * @return {Promise}
   */
  ...
  add: async (values) => {
    ...
    const entry = await Partner.create(data);

    // Create relational data and return the entry.
    return Partner.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Partner.associations.map(a => a.alias));
    const data = _.omit(values, Partner.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Partner.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Partner.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Partner.attributes).reduce((acc, curr) => {
      switch (Partner.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
